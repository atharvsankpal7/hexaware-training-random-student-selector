...
export default function StudentSelector() {
  ...
  const selectRandomStudent = () => {
    if (remainingStudents.length > 0) {
      ...
      setRemainingStudents(remainingStudents.filter(student => student !== selected))
    } else {
            ...
            <Tabs defaultValue="input" className="w-full">
              ...
              <TabsContent value="input" className="space-y-4">
                <div className="flex space-x-2 mt-4">
                  ...
                  />
                  <Button onClick={addStudent} className="bg-green-500 hover:bg-green-600 transition-colors duration-200">
              ...
              <TabsContent value="selection" className="space-y-4">
                ...
                {selectedStudent && (
                  <div
                    className="bg-indigo-100 p-4 rounded-lg text-center transform transition-all duration-300 animate-fade-in"
                  >
                    <h3 className="text-lg font-semibold text-indigo-800">Selected Student:</h3>
                    <p className="text-2xl font-bold text-indigo-600 mt-2 animate-bounce">{selectedStudent}</p>
                  </div>
                )}
                
                <div className="text-sm text-gray-600 text-center transition-opacity duration-200">
                  Remaining students: {remainingStudents.length} / {students.length}
                </div>
              </TabsContent>
            </Tabs>
          )}
